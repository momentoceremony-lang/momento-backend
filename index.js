...
app.post('/api/crm/system/migrate-gallery', async (req, res) => {
    try {
        // 1. Force table creation just in case it failed previously
        await pool.query(`
            CREATE TABLE IF NOT EXISTS gallery_submissions (
                id SERIAL PRIMARY KEY,
                photographer_id INT REFERENCES photographers(id) ON DELETE SET NULL,
        ...
        // 2. Fetch all photographers with existing galleries
        const pros = await pool.query("SELECT id, gallery FROM photographers WHERE gallery IS NOT NULL");
        let migratedCount = 0;

        for (let pro of pros.rows) {
            const galleryArray = pro.gallery; 
            if (!Array.isArray(galleryArray)) continue; // Skip if empty or corrupted

            for (let item of galleryArray) {
                const imgUrl = typeof item === 'string' ? item : item.url;
                const imgCat = typeof item === 'string' ? 'Uncategorized' : (item.category || 'Uncategorized');

                // 3. Ensure it is a valid web URL (Catches Cloudinary, Imgur, etc.)
                if (imgUrl && imgUrl.startsWith('http')) {
                    ...
                    if (check.rows.length === 0) {
                        await pool.query(
                            ...
                            [pro.id, imgUrl, imgCat.substring(0, 99)] // Safety truncate category
                        );
                        migratedCount++;
                    }
                }
